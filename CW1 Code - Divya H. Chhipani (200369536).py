...
class SRCNN(nn.Module):
    def __init__(self):
        ...
        # To calculate padding: output_size = ((input_size - kernel_size + 2*padding)/stride)+1
...
print('Size:', output_.size())
output_  = output_.cpu().numpy()
output_ = np.reshape(output_,(255,255))

groundtruth = scipy.misc.imsave('groundtruth_img.jpg', HR_image)
blurred = scipy.misc.imsave('blurred_img.jpg', LR_image)
output = scipy.misc.imsave('SR_img.jpg',output_) # SR image

# PSNR calculation
psnr_hr_bi = skimage.metrics.peak_signal_noise_ratio(HR_image, LR_image)
print('PSNR for Bicubic Interpolated HR image: ', psnr_hr_bi)
psnr_hr_srcnn = skimage.metrics.peak_signal_noise_ratio(HR_image, output_)
print('PSNR for SRCNN Super Resolution image: ', psnr_hr_srcnn)
